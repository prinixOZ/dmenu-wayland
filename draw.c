...
PangoLayout *get_pango_layout(cairo_t *cairo, const char *font,
    ...
    //PangoFontDescription *desc = pango_font_description_from_string("Hack 12");
...
static void output_geometry(void *data, struct wl_output *wl_output, int32_t x,
		int32_t y, int32_t width_mm, int32_t height_mm, int32_t subpixels,
		const char *make, const char *model, int32_t transform) {
	struct monitor_info *monitor = data;
	monitor->subpixel = subpixels;
}
...
void dmenu_show(struct dmenu_panel *dmenu) {
    ...
    struct pollfd fds[] = {
        { .fd = wl_display_get_fd(dmenu->display_info.display), .events = POLLIN, .revents = 0 },
        { .fd = dmenu->repeat_timer, .events = POLLIN, .revents = 0 },
    };
